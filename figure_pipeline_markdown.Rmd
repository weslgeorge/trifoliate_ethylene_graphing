---
title: "figure_pipeline_markdown"
...
The purpose of this document is to show an example of how I plot my ethylene data for whoever wants to use it. The bases for this comes from screening multiple M3 Cowpea plants for ethylene production under three different treatment groups. Cowpea have trifoliate leaves in which individual leaflets were used as each data point for one plant. Different experiments will have different contions and needs so adjust this template for whatever you need!

The first step to any script is to library your packages

```{r library packages, message=F}
library(dplyr) # helps with data wrangling 
library(tidyr) # helps with data wrangling, includes pivot_longer function 
library(ggplot2) # needed for plotting in ggplot2, simplifies plotting and adds additional options
library(ggpubr) # helpful for plotting in ggplot2, adds extra options to plots
library(ggrepel)
```

## Data Wrangling

First things first we need to load in the data. 

```{r loading in data}
data_file_df<- read.csv(file = "test_data.csv") # loads in data, be sure to set working directory to source file location
```

This data file has a large series of columns with each row representing an individual plant and columns for all the important test information we want to plot. Specifically we want to plot the standardized height values from the Gas Chromatograph under columns water treatment, treatment 1, and treatment 2. To do this easily in ggplot it is useful to get all the data under a single column. In the following code block I use pivot_longer to take the columns c('water_treatment', 'treatment_1', 'treatment_2'), and "pivot" them into two columns. One column is going to be called whatever we assign after the `names_to =`, basically saying which column the value was pulled from. The next column is our values and `values_to =` lets us assign a column name to the values we are pivoting.

```{r pivoting ethylene values}
...
While not necessary for my analysis in this file, it can be useful to see if there are any trends with mass and the response, the following pivots out the mass information in the same way as the ethylene information and then joins the data back to the original data frame. 
```{r pivoting mass values}
...
Now we have to make some modifications and filtering to the new dataframe, most of this is up to you, but I would reccomend having some kind of unique plant ID system if you dont already have one in your data collection. This allows you to connect the dots in later plotting

```{r general filtering and getting data ready}
...
Now lets add some factors for the treatments to go in the same order as the trifoliates they were attached to! Factors are important for getting the order left to right correct in ggplot2, we can change this order by assigning levels to the factors

```{r setting up factors}
...
Now that the data is ready we can make some plots! Ethylene data is highly variable day by day, but consistent with comparing lines, because of this I recommend doing one last filter prior to plotting for the single day experiment you want to plot as you do experiments day to day. This next plot lets you quickly check the results of your single experiment pretty well.

Just a heads up, I will try to caption each layer of the ggplot when they first appear, but otherwise try experimenting by playing around with the layers yourself to get something you like data overview plot for one experiment. 

```{r plotting for a days experiment}
...
Next up is a plot that shows all the data at once, it can be a bit harder to look at so I still recommend breaking things up until you are ready to pool the data together in the next section

```{r raw data overview plot}
# raw data overview plot for all dates tested
ggplot(all_data_long,
  ...
  geom_point(aes(color = Date.Tested, group = M2_and_M3),
             position = position_dodge(width = 0.1))+
  facet_grid(cols = vars(M2_rep), rows = vars(Date.Tested))+ # this sepearates the plots by parentage and date tested
...
For my data, we wanted to normalize the data to 'low control' plants that seemed to be homozygus for a lower ethylene response to our treatment peptide. This allowed us to compare and pool results across days and experiments since the values of ethylene are highly variable day by day, but trends tend to hold the same regardless of daily gc height values. 
...
This code will create z-scores based on the mean of the family B within each treatment type, so the water z-score is relative to the B families water treated trifoliate leaf mean and sd

## Plotting z-score values in ggplot2 

Now that we have the z-score dataframe we can begin to plot it!

```{r plotting with z-scores}
...
Now we can better compare the values across experimental days! every number value (ex, 1 2 3 ect) tells you  how mant standard deviations a trifoliate was away from the average of family Bs trifoliate with the same experimental treatment 

```{r plotting collapsed date}
...
Here is what the data looks like if we collapse everything together, It gets a bit messier so you can think about tweaking the alpha values on all the lines or points, or doing away with them if they are no longer adding anything to the plot due to clutter 


**I hope these plotting examples and z-score example helped! feel free to reach out if you have any questions!**